...
//	camera.set(CV_CAP_PROP_FRAME_WIDTH, 3040);
//	camera.set(CV_CAP_PROP_FRAME_HEIGHT, 1520);
	...
	while (true)
	{
		...
		if (27 == cv::waitKey(20))//按键盘空格键存储图像，左边从1开始，右边从26开始
		{
			string path;

			static int cnt = 0;
			cnt++;
			path = std::to_string(cnt) + ".jpg";
			cv::imwrite(path, image_l);

			path = std::to_string(cnt + 25) + ".jpg";
			cv::imwrite(path, image_r);

			printf("存储了%d帧\n", cnt);
		}
		else if (20 == cv::waitKey(20))
		{
			MessageBox(NULL, TEXT("将要推出程序，确定执行吗？"), TEXT("操作提示"), 1);
			break;
		}
